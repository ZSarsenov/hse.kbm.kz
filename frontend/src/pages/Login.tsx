    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.token) {
           ...
           setError('Ошибка: Сервер не прислал токен.');
        }
      } else {
        if (response.status === 400) {
           setError('Неверный логин или пароль');
        } else if (response.status === 404) {
           setError('URL для входа не найден (404). Проверьте urls.py');
        } else {
      ...
      console.error(err);
      setError('Ошибка соединения. Убедитесь, что Django запущен.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    <div className="fixed inset-0 h-screen w-screen overflow-hidden flex items-center justify-center font-sans text-slate-800">
      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-2xl overflow-hidden relative z-10 mx-4">
        <div className="bg-gray-50 p-8 text-center border-b border-gray-100 font-['PT_Sans']">
           <h1 className="text-4xl font-bold text-blue-900 tracking-tight">ЭНД</h1>
           <p className="text-gray-500 mt-2 text-lg font-medium">АО «Каражанбасмунай»</p>
        </div>
